const { estimateCategory, layoutVariant, showerGlassThicknessTypes, wineCellarLayoutVariants, showerWeightMultiplier } = require("@config/common");
const EstimateService = require("@services/estimate");
const LayoutService = require("@services/layout");

exports.estimateSaveFormat = async (estimateData) => {
  try {
    let estimatePayload = null;
    if (estimateData?.category === estimateCategory.SHOWERS) {
      estimatePayload = await generateEstimatePayloadForShowers(estimateData);
    } else if (estimate?.category === estimateCategory.MIRRORS) {
      estimatePayload = await generateEstimatePayloadForMirrors(estimateData);
    ...
    if (estimatePayload) {
      const estimate = await EstimateService.create(estimatePayload);
      return estimate;
    }
  } catch (err) {
    throw err;
...
const generateEstimatePayloadForShowers = async (estimateData) => {
  try {
    if(estimateData.layout.variant === layoutVariant.CUSTOM){

    } else {
      const layout = await LayoutService.findBy({_id:estimateData.layout._id});
      const glassThickness = getGlassThickness(estimateData.layout.variant,estimateData.estimateDetail.dimensions,layout?.settings?.heavyDutyOption?.height || 85);
      ...
      config.hardwareFinishes = layout?.settings?.hardwareFinishes ?? null;
      config.handles = {
                type: estimateData.estimateDetail.handle._id,
                count: layout?.settings?.handles?.count ?? 1
            }
      config.hinges = {
              type: estimateData.estimateDetail.hinge._id,
              count: layout?.settings?.hinges?.count ?? 1
          }
      config.cornerClamps = {
                wallClamp: layout?.settings?.cornerWallClamp?.wallClampType ? [{type:layout?.settings?.cornerWallClamp?.wallClampType,count:layout?.settings?.cornerWallClamp?.count}] : [],
                sleeveOver: layout?.settings?.cornerSleeveOver?.sleeveOverType ? [{type:layout?.settings?.cornerSleeveOver?.sleeveOverType,count:layout?.settings?.cornerSleeveOver?.count}] : [],
                glassToGlass: layout?.settings?.cornerGlassToGlass?.glassToGlassType ? [{type:layout?.settings?.cornerGlassToGlass?.glassToGlassType,count:layout?.settings?.cornerGlassToGlass?.count}] : []
            };
      if(layout.settings?.channelOrClamps === 'Channel'){
        ...
        config.mountingClamps = {
          wallClamp: layout?.settings?.wallClamp?.wallClampType ? [{type:layout?.settings?.wallClamp?.wallClampType,count:layout?.settings?.wallClamp?.count}] : [],
          sleeveOver: layout?.settings?.sleeveOver?.sleeveOverType ? [{type:layout?.settings?.sleeveOver?.sleeveOverType,count:layout?.settings?.sleeveOver?.count}] : [],
          glassToGlass: layout?.settings?.glassToGlass?.glassToGlassType ? [{type:layout?.settings?.glassToGlass?.glassToGlassType,count:layout?.settings?.glassToGlass?.count}] : []
      }
      }
      // let handleType = null;
      // if(layoutData?.settings?.handles?.handleType)
...
const generateEstimatePayloadForWineCellars = async (estimateData) => {
  try {
  } catch (err) {
    throw err;
  }
...
const getGlassThickness = (variant, measurementSides, height) => {
